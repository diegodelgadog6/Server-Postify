...
from app.schemas.post import PostRead, PostCreate, PostReadDetails, PostUpdate
...
async def add_comment(post_id: str, data: CommentCreate, session: AsyncSession = Depends(get_session)):
    result = await session.execute(select(Post).where(Post.id == post_id))
    post = result.scalar_one_or_none()
